...
//	}
	...
	/**
	 * 通用url解码，将%xx直接转成对应的byte，不考虑具体编码
	 * 支持%FF，%uFFFF，四种编码方法。注意，这里指的编码同"GBK"不同，不要混淆
	 * @param url  被解码的url.
	 * @param buff 用于存放解码后数据的缓存
	 * @param flag 格式扩展参数，
	 *   0表示仅针对url编码处理
	 *   第一位为1，表示需要处理apache格式的编码
	 * @return
	 *   >=0 解析成功，buff中数据的长度 
	 *   -1 参数有问题，url为空
	 *   -2 参数有问题，buff空间不足
	...
	/**
	 * 根据一串byte码流判断可能的字符编码
	 * 依赖于jchardet包，根据其提供的probe机制判断对应的编码类型，有一定的误判率
	 * 码流必须是按照一种统一的规则来编码的，码流的数据越多，判定的正确性越高
	 * @param s 待判定的码流
	 * @return 可能的编码类型数组，"UTF-8", "GBK", "UTF-16"等等.
	 *         如果参数有问题，返回值为null
	 */
